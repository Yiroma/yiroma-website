...
export function PricingCard({ plan, ariaLabel = "Inclus dans ce forfait" }: Props) {
  return (
    <Card
      className={cn(
        "relative gap-5 py-6 transition-[box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(0,0,0,0.10)]",
        plan.highlighted ? "border-primary/50 bg-primary/5 ring-primary/20" : "ring-foreground/10",
      )}
    >
      {plan.highlighted && (
